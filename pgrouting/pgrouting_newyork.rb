...
daytimes.each do |daytime|
  ...
  count = conn.exec("SELECT count(*) FROM #{schema}.bikesharing_trips_#{daytime}_grand_central_station").values.first.first.to_i
  puts "Count: #{count}"
